...
module.exports = {
    meta: {
        ...
        docs: {
            ...
            recommended: true,
            url: "https://github.com/dprgarner/eslint-plugin-no-communist-spelling/blob/master/docs/rules/no-communist-spelling.md"
        },
        ...
        messages: {
            noCommunistSpelling: "Identifier '{{word}}' is un-American. Use {{prefer}} instead."
        }
    },
        ...
        function checkSpellings(node) {
            if (reported.indexOf(node) < 0) {
                ...
                findBritishSpellings(node.name).forEach(([word, prefer]) => {
                    ...
                    nodeReported = true;
                    context.report({ node, messageId: "noCommunistSpelling", data: {word, prefer} });
                });
                if (nodeReported) {
                    reported.push(node);
                }
