...
module.exports = {
    meta: {
        ...
        docs: {
            ...
            recommended: true,
            url: "https://github.com/dprgarner/eslint-plugin-communist-spelling/blob/master/docs/rules/communist-spelling.md"
        },
        ...
        messages: {
            communistSpelling: "The spelling '{{word}}' in '{{name}}' is anti-American. Use '{{prefer}}' instead."
        }
    },
    ...
    create(context) {
        ...
        function checkSpellings(node) {
            if (reported.indexOf(node) < 0) {
                ...
                findBritishSpellings(node.name).forEach(([word, prefer]) => {
                    if (isAllowed(word)) {
                        return;
                    }
                    nodeReported = true;
                    context.report({ node, messageId: "communistSpelling", data: {name: node.name, word, prefer} });
                });
                if (nodeReported) {
                    reported.push(node);
                }
            ...
            Identifier(node) {
                 ...
                 */
                    ...
                    if (node.parent.parent && node.parent.parent.type === "ObjectPattern") {
                        ...
                        if (node.parent.value.name && !(assignmentKeyEqualsValue && ignoreDestructuring)) {
                            checkSpellings(node);
                        }
                    }
                    ...
                    if (!ALLOWED_PARENT_TYPES.has(effectiveParent.type) && !(node.parent.right === node)) {
                        checkSpellings(node);
                    }
